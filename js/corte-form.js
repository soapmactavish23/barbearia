...
if (data) {
	$('.modal-title').text('Corte #' + data.idcorte);
	$('input[name="idcorte"]').val(data.idcorte);
...
$('form').submit(function () {
	...
	formData.push({ name: 'classe', value: 'corte' });
	...
	$.ajax({
		...
		success: function (result) {
			if (result.error) {
				...
				$('.modal-title').text('Corte #' + result.idcorte);
				$('input[name="idcorte"]').val(result.idcorte);
				$('#btn-excluir').show();
				alert('Corte ID ' + result.idcorte + ' gravado!');
...
$('#btn-excluir').click(function () {
	if (confirm('Tem certeza que deseja excluir este registro?')) {
		...
		formData.push({ name: "token", value: token });
		formData.push({name: 'idcorte', value: $('input[name="idcorte"]').val()});
		$.ajax({
			type: "POST",
			url: url + '/api.php',
			data: formData,
			success: function (result) {
				if (result.error) {
					...
					$('.modal').modal('hide');
					alert('ID '+result.idcorte+' excluído!');
					datatable.ajax.reload(null, false);
				}
			}
		});
